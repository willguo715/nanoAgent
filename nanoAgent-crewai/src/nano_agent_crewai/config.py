...
def _package_project_root() -> Path:
    """本文件位于 src/nano_agent_crewai/config.py，项目根为 nanoAgent-crewai 目录。"""
...
def _resolve_config_file(requested: str) -> tuple[Path, bool]:
    ...
    raise FileNotFoundError(
        f"配置文件不存在: {requested}。"
        " 请将 nanoAgent-crewai/config.yaml.example 复制为 config.yaml 并填写密钥，"
...
def _inherit_tavily_from_repo_root_if_empty(cfg: dict[str, Any], loaded_path: Path) -> None:
    """
    子项目 nanoAgent-crewai 的 config.yaml 若未填 tavily.api_key，
    ...
    if resolved.parent.name != "nanoAgent-crewai":
        return
...
def _inherit_llm_from_repo_root_if_empty(cfg: dict[str, Any], loaded_path: Path) -> None:
    """
    子项目 nanoAgent-crewai 的 llm.api_key 为空时，
...
def ensure_llm_credentials(cfg: dict[str, Any]) -> None:
    """在调用 LLM 前校验密钥已配置；否则抛出带说明的 ValueError。"""
    llm = cfg.get("llm") or {}
    if str(llm.get("api_key") or "").strip():
        return
    raise ValueError(

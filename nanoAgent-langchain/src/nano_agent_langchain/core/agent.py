...
- 在 Windows 上禁止使用 Linux 的 date +%A、date +%F 等（CMD 会卡住或超时）；请用 python -c 打印本机日期与星期，例如：python -c "import datetime; n=datetime.datetime.now(); print(n.strftime('%Y-%m-%d'), n.strftime('%A'))"
- 当用户询问「今天/现在/几月/几号/几点/星期几/日期/时间」等依赖本机或实时环境的信息时，必须 need_tools=true，并用 shell_exec 获取本机日期时间，不可仅凭模型记忆回答。
...
class LangChainAgentRunner:
    ...
    def _plan(
        ...
        fb = (execution_feedback or "").strip()
        retry_hint = ""
        if fb and fb != "(none)":
            retry_hint = (
                "\n【上一轮未通过，必须按反馈修正】\n反馈原文："
                + fb
                + "\n若反馈指出「需要工具证据但未调用工具」：本轮必须设 need_tools=true，"
                "并至少调用一次 shell_exec 或 tavily_search；禁止再次仅返回 need_tools=false。\n"
            )

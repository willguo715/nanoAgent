...
from nano_agent_langgraph.config import ensure_llm_credentials, load_config
from nano_agent_langgraph.core.graph_agent import LangGraphAgentRunner
...
def run_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"[nano-agent-langgraph] 配置错误：{exc}", file=sys.stderr)
        raise SystemExit(2) from exc
    runner = LangGraphAgentRunner(cfg, config_path=args.config)
    print("[nano-agent-langgraph] 任务开始...")
    ...
    print("[nano-agent-langgraph] 任务结束，以下是最终报告：\n")
    print(report)
...
def chat_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise SystemExit(2) from exc
    runner = LangGraphAgentRunner(cfg, config_path=args.config)
    print(f"[nano-agent-langgraph] chat started, session={args.session}, 输入 exit 退出。")
    while True:
        prompt = input("\n你> ").strip()
        if prompt.lower() in {"exit", "quit"}:
            print("[nano-agent-langgraph] bye.")
            break
        image = input("图片路径(可留空)> ").strip()
        image = _clean_image_path(image or None)
        print("[nano-agent-langgraph] 开始处理...")
        report = runner.run_once(
            session_name=args.session,
            user_input=prompt,
            image_path=image,
        ...
        print("[nano-agent-langgraph] 处理完成。")
        print("\n" + report + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="nano-agent-langgraph",
        description="基于 LangGraph 的本地 CLI agent（与 nano-agent 行为对齐）。",
    )

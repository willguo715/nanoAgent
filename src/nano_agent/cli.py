...
from nano_agent.config import load_config
from nano_agent.core.agent import AgentRunner
...
def run_command(args: argparse.Namespace) -> None:
    ...
    runner = AgentRunner(cfg, config_path=args.config)
    print("[nano-agent] 任务开始...")
...
def chat_command(args: argparse.Namespace) -> None:
    ...
    cfg = load_config(args.config)
    runner = AgentRunner(cfg, config_path=args.config)
    print(f"[nano-agent] chat started, session={args.session}, 输入 exit 退出。")
    while True:
        prompt = input("\n你> ").strip()
        if prompt.lower() in {"exit", "quit"}:
            print("[nano-agent] bye.")
            break
        image = input("图片路径(可留空)> ").strip()
        image = _clean_image_path(image or None)
        print("[nano-agent] 开始处理...")
        report = runner.run_once(
            session_name=args.session,
            user_input=prompt,
            image_path=image,
        ...
        print("[nano-agent] 处理完成。")
        print("\n" + report + "\n")


def build_parser() -> argparse.ArgumentParser:
    """构建 CLI 参数解析器并绑定子命令处理函数。"""
    parser = argparse.ArgumentParser(prog="nano-agent", description="A local CLI agent.")
